...
Status Canbus::Init() {
  ...
  AINFO << "Canbus_conf:" << canbus_conf_.ShortDebugString();
...
void Canbus::CheckStatus() {

  uint8_t status = vehicle_controller_->getstatus();
  if ((status & 0b00000001)==0)
      AWARN<<"GNSS posiotion not init";
    if ((status & 0b00000010)==0)
      AWARN<<"GNSS speed not init";
      if ((status & 0b00000100)==0)
      AWARN<<"GNSS atittude not init";
      if ((status & 0b00001000)==0)
      AWARN<<"GPS posiotion signal not good";
      if ((status & 0b00010000)==0)
      AWARN<<"GPS speed signal not good";
      if ((status & 0b00100000)==0)
      AWARN<<"GPS lost heading";
      if ((status & 0b01000000)==0)
      AWARN<<"GNSS lost wheel speed";
}

void Canbus::PublishLocalization() {
  LocalizationEstimate localization = vehicle_controller_->localization();
   double x = localization.pose().position().x();//log
    double y = localization.pose().position().y();//lat
    _wgs84pj_source = pj_init_plus(FLAGS_WGS84_TEXT.c_str());
    _utm_target = pj_init_plus(FLAGS_UTM_TARGET.c_str());
    x=125.35; y=43.88;
    double DEG_TO_RAD_LOCAL = M_PI / 180.0;
    x *= DEG_TO_RAD_LOCAL;
    y *= DEG_TO_RAD_LOCAL;
    int ret = pj_transform(_wgs84pj_source, _utm_target, 1, 1, &x, &y, NULL);
    if (ret != 0) {
        AERROR<<"prj transform failed, x: " << x << ", y: " << y
                         << ", ret: " << ret;
    }

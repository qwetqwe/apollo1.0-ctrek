...
//pad msg
const uint32_t KEYCODE_LEFT = 0x6A;   // 'j'
const uint32_t KEYCODE_RIGHT = 0x6B;  // 'K'
const uint32_t KEYCODE_NONE = 0x6C;   // 'l'
const uint32_t KEYCODE_WARNBLINK = 0x70 ;  // 'p'
const uint32_t KEYCODE_WARNBLINK_OFF = 0x6F ;  // 'p'
class Teleop {
 public:
  Teleop();
    ...
    while (IsRunning()) {
      ...
      switch (c) {
        ...
        case KEYCODE_HELP2:
          ...
          break;
        case KEYCODE_LEFT:
          control_command_.mutable_signal()->set_turn_signal(::apollo::canbus::Signal::TURN_LEFT);
          break;
        case KEYCODE_RIGHT:
          control_command_.mutable_signal()->set_turn_signal(::apollo::canbus::Signal::TURN_RIGHT);
          break;
        case KEYCODE_NONE:
          control_command_.mutable_signal()->set_turn_signal(::apollo::canbus::Signal::TURN_NONE);
          break;
        case KEYCODE_WARNBLINK:
          control_command_.mutable_signal()->set_emergency_light(true);
          break;
        case KEYCODE_WARNBLINK_OFF:
          control_command_.mutable_signal()->set_emergency_light(false);
          break;
        default:
          // printf("%X\n", c);
          break;
  ...
  void GetPadMessage(PadMessage& pad_msg, int32_t int_action) {
    ...
    switch (int_action) {
      case 0:
        ...
        pad_msg.set_driving_mode(Chassis::COMPLETE_MANUAL);
        printf("SET Action RESET\n");
        break;
      case 1:
        action = apollo::control::DrivingAction::START;
        pad_msg.set_driving_mode(Chassis::COMPLETE_AUTO_DRIVE);
  ...
  void Send() {
    ...
    AINFO << "Control Command send OK:" << control_command_.ShortDebugString();
  }

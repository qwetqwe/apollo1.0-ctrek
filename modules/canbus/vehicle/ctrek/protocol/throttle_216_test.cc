...
TEST(Throttle216Test, General) {
  uint8_t data[8] = {0x67, 0x68, 0x63, 0x64, 0x51, 0x52, 0x53, 0x54};
  Throttle216 throttle;
  EXPECT_EQ(throttle.GetPeriod(), 10 * 1000);
  throttle.set_pedal(99.45);
  throttle.set_enable();
  throttle.UpdateData(data);
  EXPECT_EQ(data[0], 0b00000001);
  EXPECT_EQ(data[1], 0b11111111);

...
class ctrekControllerTest : public ::testing::Test {
 public:
  virtual void SetUp() {
...
TEST_F(ctrekControllerTest, Status) {
  controller_.Init(params_, &sender_, &msg_manager_);
...
TEST_F (ctrekControllerTest, UpdateDrivingMode) {
  controller_.Init(params_, &sender_, &msg_manager_);
  controller_.set_driving_mode(Chassis::COMPLETE_AUTO_DRIVE);

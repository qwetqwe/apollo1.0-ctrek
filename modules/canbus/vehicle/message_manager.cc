...
void MessageManager::Parse(const uint32_t message_id, const uint8_t* data,
  ...
  } else if (message_id==0x506){
    protocol_data->Parse(data, length, &status_);
    }
    else{

    // TODO: only lincoln implemented this virtual function
    std::lock_guard<std::mutex> lock(chassis_detail_mutex_);
    protocol_data->Parse(data, length, &localization_);
  }
...
ErrorCode MessageManager::GetLocalization(LocalizationEstimate* const localization)
{
  if (localization == nullptr) {
    AERROR<<"Failed to get localization due to nullptr.";
    return ErrorCode::CANBUS_ERROR;
  }
  std::lock_guard<std::mutex> lock(chassis_detail_mutex_);
  localization->CopyFrom(localization_);
  return ErrorCode::OK;
}
ErrorCode MessageManager::GetStatus(uint8_t* const status)
{
  if (status == nullptr) {
    AERROR<<"Failed to get localization due to nullptr.";
    return ErrorCode::CANBUS_ERROR;
